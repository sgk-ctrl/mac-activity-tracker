...
def test_build_produces_html(tmp_path):
    data = {"source": "sample", "window_start": "2026-01-01", "window_end": "2026-01-14",
            "generated_at": "2026-01-14T10:00:00", "sessions": [
                {"start": "2026-01-01T09:00", "name": "VS Code", "kind": "app",
                 "category": "Coding", "is_agentic": False, "minutes": 30}]}
...
def test_script_injection_is_neutralized(tmp_path):
    ...
    data = {"source": "live", "window_start": "2026-01-01", "window_end": "2026-01-14",
            "generated_at": "2026-01-14T10:00:00", "sessions": [
                {"start": "2026-01-01T09:00", "name": payload, "kind": "web",
                 "category": "Social", "is_agentic": False, "minutes": 3}]}
    out = tmp_path / "dash.html"
    build_dashboard.build(_write(tmp_path, data), str(out), "vendor/chart.umd.min.js")
    html = out.read_text()
    ...
    m = re.search(r'<script id="activity-data" type="application/json">(.*?)</script>',
                  html, re.S)
...
def test_data_token_collision(tmp_path):
    ...
    data = {"source": "live", "window_start": "2026-01-01", "window_end": "2026-01-14",
            "generated_at": "2026-01-14T10:00:00", "sessions": [
                {"start": "2026-01-01T09:00", "name": "__DATA__", "kind": "app",
                 "category": "Other", "is_agentic": False, "minutes": 1}]}
    out = tmp_path / "dash.html"
    build_dashboard.build(_write(tmp_path, data), str(out), "vendor/chart.umd.min.js")
    html = out.read_text()
